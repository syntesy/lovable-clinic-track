...
import {
  Search, BookOpen, ExternalLink, FileText, Filter,
  ChevronLeft, ChevronRight, Loader2, X, Heart,
} from "lucide-react";
import {
  useAcademyArticles, useAcademyArticleDetail, useArticleFilterOptions,
  type ArticleFilters,
...
export default function ExplorarTabContent() {
  ...
  const updateParam = useCallback((key: string, value: string | null) => {
    setSearchParams(prev => {
      ...
      // preserve tab
      if (!next.has("tab")) next.set("tab", "explorar");
      return next;
    });
  }, [setSearchParams]);
  ...
  const clearFilters = useCallback(() => {
    setSearchParams({ tab: "explorar" });
    setSearchInput("");
  }, [setSearchParams]);

  const hasFilters = Array.from(searchParams.entries()).some(([k]) => k !== "tab");
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <Select value={filters.study_type ?? "all"} onValueChange={(v) => updateParam("study_type", v === "all" ? null : v)}>
                <SelectTrigger className="w-[180px]"><SelectValue placeholder="Tipo de estudo" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">Todos os tipos</SelectItem>
                  {filterOptions?.studyTypes.map((t) => (<SelectItem key={t} value={t}>{t}</SelectItem>))}
                </SelectContent>
              </Select>
              <Select value={filters.sort ?? "recent"} onValueChange={(v) => updateParam("sort", v)}>
              ...
              <Button variant={onlySaved ? "default" : "outline"} size="sm" className="gap-1" onClick={() => updateParam("saved", onlySaved ? null : "1")}>
                <Heart className={`w-3 h-3 ${onlySaved ? "fill-current" : ""}`} /> Salvos
              </Button>
              {hasFilters && (
                <Button variant="ghost" size="sm" onClick={clearFilters} className="gap-1"><X className="w-3 h-3" /> Limpar</Button>
              )}
            </div>
          </div>
        </div>
      ...
      <section className="py-8">
        <div className="container mx-auto px-4">
          {isLoading ? (
            ...
            <>
              <p className="text-sm text-muted-foreground mb-6">{total} artigo{total !== 1 ? "s" : ""} encontrado{total !== 1 ? "s" : ""}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {articles.map((article) => {
                  const isFav = favorites.includes(article.id);
                  return (
                    <Card key={article.id} className="group hover:shadow-lg transition-all cursor-pointer flex flex-col" onClick={() => setSelectedArticleId(article.id)}>
                      <CardContent className="py-5 flex flex-col flex-1">
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex flex-wrap gap-1.5">
                            ...
                            {article.interventions.slice(0, 2).map((i) => (<Badge key={i} variant="outline" className="text-xs">{i}</Badge>))}
                            {article.pathologies.slice(0, 1).map((p) => (<Badge key={p} variant="secondary" className="text-xs">{p}</Badge>))}
                          </div>
                          <button onClick={(e) => { e.stopPropagation(); toggleFav.mutate({ articleId: article.id, isFavorited: isFav }); }} className="ml-2 flex-shrink-0">
                        ...
                        <p className="text-xs text-muted-foreground mb-3">{article.year} • {article.journal}</p>
                        <p className="text-sm text-muted-foreground line-clamp-3 flex-1">{article.summary_short}</p>
                        <div className="flex gap-2 mt-4 pt-3 border-t">
                          <Button variant="outline" size="sm" className="flex-1 gap-1" onClick={(e) => { e.stopPropagation(); setSelectedArticleId(article.id); }}>
                            <BookOpen className="w-3.5 h-3.5" /> Ver resumo
                          </Button>
                          {getExternalUrl(article) && (
                            <Button variant="ghost" size="sm" className="gap-1" onClick={(e) => { e.stopPropagation(); window.open(getExternalUrl(article)!, "_blank"); }}>
          ...
          {isLoadingDetail ? (
            ...
            <ScrollArea className="max-h-[75vh] pr-4">
              <DialogHeader className="mb-4">
                <Badge className={`w-fit mb-2 ${getStudyBadgeClass(articleDetail.study_type)}`}>{articleDetail.study_type}</Badge>
                <DialogTitle className="text-xl leading-tight">{articleDetail.title}</DialogTitle>
              ...
              <div className="space-y-5">
                <div className="flex flex-wrap gap-1.5">
                  {articleDetail.interventions.map((i) => (<Badge key={i} variant="outline">{i}</Badge>))}
                  {articleDetail.pathologies.map((p) => (<Badge key={p} variant="secondary">{p}</Badge>))}
                </div>
                {articleDetail.summary_full ? (
                  <div>
                    <h4 className="text-sm font-semibold text-foreground mb-2">Resumo Completo</h4>
                    <div className="text-sm text-muted-foreground whitespace-pre-wrap">{articleDetail.summary_full}</div>
                  </div>
                ) : (
                  <div>
                ...
                {articleDetail.limitations && articleDetail.limitations.length > 0 && (
                  <div>
                    ...
                    <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
                      {articleDetail.limitations.map((l, i) => (<li key={i}>{l}</li>))}
                    </ul>
                  </div>
                )}
                ...
                {getExternalUrl(articleDetail) && (
                  <Button variant="outline" className="gap-2 w-full" onClick={() => window.open(getExternalUrl(articleDetail)!, "_blank")}>
                    <ExternalLink className="w-4 h-4" /> Abrir no {articleDetail.pubmed_url ? "PubMed" : "DOI"}
                  </Button>
                )}
              </div>
            </ScrollArea>
