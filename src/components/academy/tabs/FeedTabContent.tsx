...
import { Loader2, Sparkles, Clock, FolderOpen, ArrowRight, BookOpen, Heart } from "lucide-react";
        ...
        <CardContent className="py-5 flex flex-col flex-1">
          <div className="flex items-start justify-between mb-2">
            <div className="flex flex-wrap gap-1.5">
              {isNew(article.created_at) && <Badge className="bg-primary text-primary-foreground text-[10px]">Novo</Badge>}
              <Badge className={getStudyBadgeClass(article.study_type)}>{article.study_type}</Badge>
              {article.interventions?.slice(0, 2).map((i: string) => (<Badge key={i} variant="outline" className="text-xs">{i}</Badge>))}
            </div>
        ...
        <div className="container mx-auto px-4 space-y-12">
          <div>
            ...
            {loadingNew ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{newArticles.map(renderCard)}</div>
            )}
          </div>
          ...
          <div>
            ...
            {loadingForYou ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{forYou.map(renderCard)}</div>
            )}
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {officialCollections.map((col) => (
                  <Card key={col.id} className="cursor-pointer hover:shadow-lg transition-all" onClick={() => navigate(`/academy/colecoes/${col.id}`)}>
                    ...
                    <CardContent>
                      <Button variant="ghost" size="sm" className="gap-1">Ver coleção <ArrowRight className="w-3 h-3" /></Button>
                    </CardContent>
                  </Card>
                ))}
              </div>
