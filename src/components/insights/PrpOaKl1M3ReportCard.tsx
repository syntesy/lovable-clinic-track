/**
 * PrpOaKl1M3ReportCard — Reusable report component for PRP in OA KL1 with M3 follow-up.
 * Used in both PerformanceDashboard (CLINIC) and CollectiveDashboard (COLLECTIVE).
 */
...
import { Info, TrendingDown, TrendingUp, Activity, Users, AlertTriangle, BarChart3 } from 'lucide-react';
import { PrpOaKl1M3Report, ReportScope } from '@/hooks/usePrpOaKl1M3Report';
import {
...
interface Props {
  data: PrpOaKl1M3Report | undefined;
  isLoading: boolean;
  error: Error | null;
  scope: ReportScope;
}

export function PrpOaKl1M3ReportCard({ data, isLoading, error, scope }: Props) {
  if (isLoading) {
    return (
      ...
      <Card className="bg-muted/50">
        <CardContent className="py-12 text-center">
          ...
          <p className="text-muted-foreground max-w-md mx-auto">
            {data?.nTotalEligible
              ? `${data.nTotalEligible} caso(s) elegível(eis) encontrado(s), mas nenhum com baseline e follow-up M3 completos.`
              : 'Nenhum caso de PRP em Artrose KL1 com diagnóstico confirmado encontrado.'
            }
          </p>
        </CardContent>
      </Card>
  ...
  // For COLLECTIVE scope, check k-anonymity
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard
          label="N incluídos"
          value={data.nIncluded.toString()}
          icon={<Users className="h-4 w-4" />}
        />
        <KpiCard
          label="Δ Dor média"
          value={deltaPainStats.mean.toFixed(1)}
          subtitle={`Mediana: ${deltaPainStats.median.toFixed(1)} · DP: ${deltaPainStats.stdDev.toFixed(1)}`}
      ...
      {deltaFunctionStats.n > 0 && (
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Positivo = melhora · n={deltaFunctionStats.n}</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  {protocolBreakdown.map((pb) => (
                    <tr key={pb.protocolId} className="border-b hover:bg-muted/50">
                      ...
                      <td className="text-center py-2 px-2">
                        <Badge variant="secondary">{pb.n}</Badge>
                      </td>
                      <td className="text-center py-2 px-2">
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-card">
                  <tr className="border-b">
                    ...
                    <th className="text-left py-2 px-2">EVA M3</th>
                ...
                <tbody>
                  {data.cases.map((c, i) => (
                    <tr key={i} className="border-b hover:bg-muted/50">
                      ...
                      <td className="py-2 px-2">{c.m3Pain}</td>
                      <td className="text-center py-2 px-2">
