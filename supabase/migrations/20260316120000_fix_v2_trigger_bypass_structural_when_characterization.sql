-- Fix validate_attendance_pathology_v2:
-- Skip structural_grade / imaging_method requirement when characterization_json is present.
-- The characterization system replaces the legacy structural_grade fields.
...
BEGIN
  ...
  -- Structural validation only for CONFIRMED with a non-NONE model
  ...
  -- If characterization_json is present, skip legacy structural_grade validation entirely
  IF NEW.characterization_json IS NOT NULL THEN
    RETURN NEW;
  END IF;

  -- All non-NONE CONFIRMED models without characterization require structural_grade and imaging_method
